...
def ILP_LAS2(H, S, A, B, C, mem, up, down, col, com, theta, p, pt, c, d, e, f, g, s,beta):
	...
	for j in S:
		for t in H:
			prob += C[j] - ( (e * plp.lpSum(x[t1,i,j] for i in A for t1 in range(0,t+1))) + (f * plp.lpSum(y[t1,i,j,k] for i in A for k in B for t1 in range(0,t+1))) + (pt * g * plp.lpSum(z[t1,i,j] for i in A for t1 in range(0,t+1))) + (d * (t+1)) ) + (c * plp.lpSum((1 - s[t1,j]) for t1 in range(0,t+1))) <= beta[j]
