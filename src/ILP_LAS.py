...
def ILP_LAS(H, S, A, B, C, mem, up, down, col, com, theta, p, pt, c, d, e, f, g, s_mapped, beta):
    ...
    for j in S:
        for t in H:
            # Using .get() for Sparse Shadow Data
            current_battery = (
                C[j]
                - e * plp.lpSum(x[(t1,i,j)] for (t1,i,jj) in x if jj == j and t1 <= t)
                - f * plp.lpSum(y[(t1,i,j,k)] for (t1,i,jj,k) in y if jj == j and t1 <= t)
    ...
    try:
        ...
        # Fallback to command line if API fails (slower)
        print(f"Gurobi API failed: {e}. Trying CMD...")
    ...
    if status == 1: # 1 is 'Optimal' in PuLP
        print("\n--- RESULTS ---")
        for (t,i,j), var in x.items():
            if var.value() == 1: print(f"COLLECT: T={t} Sat={j} Area={i}")
        for (t,i,j), var in z.items():
            if var.value() == 1: print(f"PROCESS: T={t} Sat={j} Area={i}")
        for (t,i,j,k), var in y.items():
            if var.value() == 1: print(f"DOWNLNK: T={t} Sat={j} Area={i} -> Station={k}")
